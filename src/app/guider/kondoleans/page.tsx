 ...
 */
export default async function KondoleansPage() {
  const page = getStaticPage(SLUG)
  if (!page) notFound()

  const products = (
    await Promise.all(
      (['vita-blommor','tackblommor','liljor'] as SubCategory[]).map((sub) => getProductsBySubCategory(sub, 12))
    )
  ).flat()
